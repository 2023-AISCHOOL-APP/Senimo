...
router.get('/getSchedules/:club_code', (req, res) => {
  ...
  const query = `
  SELECT s.sche_code, s.club_code, s.sche_title, s.sche_content, s.sche_date, s.sche_location, s.fee, s.max_num, COUNT(j.user_id) AS joined_members,CONCAT('${config.baseURL}/uploads/', s.sche_img) AS sche_img, c.club_name
